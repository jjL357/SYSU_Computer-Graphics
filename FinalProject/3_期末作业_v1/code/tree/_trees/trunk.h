...
public:
   ...
   void create_positions(){
      ...
      const unsigned int nb_vertices = 6;
      ...
      index = 0;
      for (size_t i = 0; i < nb_vertices; i++) {
         unsigned int start_idx_face = 2+2*i; // 12 points per face
         unsigned int up_left = start_idx_face+1; //may be another one
         unsigned int up_right = start_idx_face+3;
         if(up_right >= 2+2*nb_vertices){
            up_right = 2+1;
         }
         unsigned int down_left = start_idx_face+0;
         unsigned int down_right = start_idx_face+2;
   ...
   void init(GLuint pid, GLuint _vao){
      ...
      //add_sub_trunks_lsystem_test(t, 0, 4, true);
      add_sub_trunks_lsystem_random(t, 0, 4, true);
   }
   void add_sub_trunks_lsystem_test(Transform t, unsigned int level, unsigned int max_level, bool is_end_point) {
       if (level >= max_level) {
           transf.push_back(t);
           ...
           if (is_end_point) {
               ...
               // position of a new point is a translation matrix
       ...
       t0.scale(1, 1, 1);
       t0.mult(t);
       add_sub_trunks_lsystem(t0, level + 1, max_level, true);
       int numBranches = 3;
       float angleInterval = -3.14159 / 3;
       float branchScale = 0.8;
       for (int i = -numBranches / 2; i <= numBranches/2; ++i) {
           //if (i == 0) continue;
           Transform branchTransform1;
           // Set the translation for the branch
           branchTransform1.translate(0, 1, 0);
           // Set the rotation for the branch
           float angle = i * angleInterval;
           branchTransform1.rotate(1.0f, 0.0f, 0.0f, angle);
           // Set the scale for the branch
           branchTransform1.scale(branchScale-0.1*(i+ numBranches / 2), branchScale, branchScale - 0.1 * (i + numBranches / 2));
           // Multiply the branch transformation with the parent transformation
           branchTransform1.mult(t);
           // Recursively generate sub-branches
           add_sub_trunks_lsystem_test(branchTransform1, level + 1, max_level, true);
           Transform branchTransform2;
           // Set the translation for the branch
           branchTransform2.translate(0, 1, 0);
           branchTransform2.rotate(0.0f, 0.0f, 1.0f, angle);
           // Set the scale for the branch
           branchTransform2.scale(branchScale - 0.1 * (i + numBranches / 2), branchScale, branchScale - 0.1 * (i + numBranches / 2));
           // Multiply the branch transformation with the parent transformation
           branchTransform2.mult(t);
           // Recursively generate sub-branches
           add_sub_trunks_lsystem_test(branchTransform2, level + 1, max_level, true);
       }
   }

   //fixed lsystem trunks
   //for debug/showing purposes
   void add_sub_trunks_lsystem(Transform t, unsigned int level, unsigned int max_level, bool is_end_point){
      if(level >= max_level){
         transf.push_back(t);
   ...
   void add_sub_trunks_fractal(Transform t, unsigned int level, unsigned int max_level){
...
protected:
   ...
   GLfloat *text_coord;
   unsigned int nb_text_coord;

   GLfloat *normals;
   unsigned int nb_normals;
};
